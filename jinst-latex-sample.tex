...
\newcommand\jml[1]{\color{magenta}JML: - #1 - \color{black}}
...
Quality assurance checks allow to ensure that the dose delivery is consistent with the prescription. However, beam monitoring systems which are currently implemented in clinical facilities are based on ionization chambers matrices, which are not designed to monitor the Bragg peak position. Indeed, they are dedicated to checking the fluence and the beam shape. In this context, clinically applicable methods and instruments for measuring the ion range and the dose profile \textit{in situ} during treatments with millimeter accuracy are under development. Some prototypes exploit the detection of $\gamma$ rays which are emitted during  relaxation  processes of excited nuclei produced by nuclear interactions (prompt-gammas (PG)~\cite{Krimmer2018_nima} or as a result of the annihilation of positrons emitted from $\beta$+ emitters~\cite{Parodi2015}. Other preliminary studies are focused on the ultrasound imaging to monitor the dose delivery thanks to the detection of shock waves induced by ion beams in the area of the Bragg peak \cite{Hickling2018}. Offline PET/CT range verification is currently applied to provide an indirect monitoring~\cite{Parodi2016_nima}. Unconventional PET-based systems using $\gamma\gamma$ coincidences are able to detect possible range deviations and unpredictable anatomical modifications during fractionated treatments~\cite{Crespo2005, Parodi2015}. Systems using time-of-flight (TOF) measurement are under development to ensure in-beam monitoring since the correlation between PG profile and the Bragg peak position has been demonstrated~\cite{Testa2008}. Prompt Gamma Timing detectors (PGT) exploit the correlation between proton range and transit time to reconstruct the Bragg peak position~\cite{Marcatili2020}, whereas TOF-PG cameras or PG spectroscopy reject background contamination mainly induced by neutrons  by means of TOF selection~\jml{REF}. 

The accelerator radio-frequency (RF) is not an ideal time reference due to the presence of the degrader in the beam axis which introduces time dispersion of bunches. Minimal variations of the cyclotron's magnetic field also slightly affect the orbital frequency of the accelerated protons. A small phase shift per turn appears between proton movement and RF and may add up to a measurable mismatch which varies in time \cite{Werner2019}. Several devices based on scintillating fibers provide particles tracking with integration \cite{Leverington2018} or particle-per-particle \cite{Horikawa2004, Achenbach2008, Braccini2012} acquisition mode and are under development for various field of application, including the proton radiography \cite{Presti2013} and the hadrontherapy \cite{PAPA2016}. Recently, the performance of a time-tracker for a prompt-gamma spectroscopy system allowing for a background TOF rejection with a sub-nanosecond time resolution has been demonstrated \cite{Martins2020}.
